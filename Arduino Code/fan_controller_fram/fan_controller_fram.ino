...
struct rgb_profile {  // 72 bytes?
  uint8_t link;  // Link to other RGB channels
  bool use_random;  // true - next colour randomly chosen, false - next in sequence
  bool smooth;  // true - smoothly fade between colours, false - jump to next colour
...
// ln rand   smo   br   sp sl rat   tr colours
  {0, false, true, 0.4, 0, 6, 0.05, 0, {4095, 4095, 0, 0, 0, 4095, 0, 0, 0, 0},
                                       {0, 4095, 4095, 4095, 0, 0, 0, 0, 0, 0},
                                       {0, 0, 0, 4095, 4095, 4095, 0, 0, 0, 0}},
  {0, true, true, 0.4, 0, 2, 0.05, 0, {0, 0, 0, 4095, 0, 0, 0, 0, 0, 0},  // Example of smooth random fade
                                      {0, 0, 0, 4095, 170, 0, 0, 0, 0, 0},
                                      {0, 0, 0, 4095, 0, 170, 0, 0, 0, 0}},
  {0, false, false, 0.4, 0, 6, 0.05, 0, {4095, 4095, 0, 0, 0, 4095, 0, 0, 0, 0},  // Example of rainbow jump
                                        {0, 4095, 4095, 4095, 0, 0, 0, 0, 0, 0},
                                        {0, 0, 0, 4095, 4095, 4095, 0, 0, 0, 0}},
};
...
const uint8_t fan_order[] = {6, 7, 0, 5, 3, 4, 1, 2};  // THIS IS WRONG FIX IT
...
void cmd_write_rgb() {
  // Function to read serial sequence of bytes into RGB profile
  static uint8_t p;
...
//  Serial.print(F("LED"));
  ...
  if (serial_sub == 1) {
    if (Serial.available() >= 1) {
      ...
      p = Serial.read();  // Get profile number
      if (p > 3) {  // If invalid profile
      //    Serial.println("Invalid RGB Profile");
        Serial.write(s_err);
        serial_func = 0;  // Reset serial to begin new function
        return;
      }
  ...
  if (serial_sub == 2) {
    ...
    if (Serial.available() >= 12) {
      // Read data into selected profile
      rgb[p].link = Serial.read();
      rgb[p].use_random = Serial.read();
      rgb[p].smooth = Serial.read();
      rgb[p].brightness = read_serial_float();
      rgb[p].seq_pos = 0;  // Reset position in sequence
      rgb[p].seq_len = Serial.read();
      rgb[p].rate = read_serial_float();
      rgb[p].transition = 0.0;  // Reset transition
  ...
  if (serial_sub >= 3) {
    if (Serial.available() >= 6) {
      // 6 bytes each time
      rgb[p].red[serial_sub - 3] = read_serial_uint16();
      rgb[p].green[serial_sub - 3] = read_serial_uint16();
      rgb[p].blue[serial_sub - 3] = read_serial_uint16();
      Serial.write(s_ack);
      serial_sub += 1;
    }
    if (serial_sub > 12) {
...
void cmd_write_fan() {
  // Function to read serial data into fan profile (also specified over serial)
  static uint8_t p;
  ...
  if (serial_sub == 2) {
    // Step 2 - get pwm, off, max_volt, changerate, hysteresis, default temp
    //        - NOTE: last speed and target speed are NOT UPDATED
    // 7 bytes
    if (Serial.available() >= 7) {
      fan[p].pwm = Serial.read();
      fan[p].allow_off = Serial.read();
      fan[p].max_voltage = read_serial_uint16();
      fan[p].changerate = Serial.read();
      fan[p].hysteresis = Serial.read();
      fan[p].last_temperature = 0;  // Trick to ensure that fan speed is updated immediately
      fan[p].default_temp = Serial.read();
      Serial.write(s_ack);
      serial_sub = 3;
    }
  }
  ...
  if (serial_sub == 3) {
    // Step 3 - Receive factors
    // 8 bytes
    if (Serial.available() >= 8) {
      for (int i = 0; i < 8; i++) {
        fan[p].factor[i] = Serial.read();
      }
      Serial.write(s_ack);
      serial_sub = 4;
    }
  ...
  if (serial_sub == 4) {
    // Step 4 - Receive temperatures
    // 4 bytes
    if (Serial.available() >= 4) {
      for (int i = 0; i < 4; i++) {
        fan[p].temperature[i] = Serial.read();
      }
      Serial.write(s_ack);
      serial_sub = 5;
    }
  ...
  if (serial_sub == 5) {
    // Step 5 - Receive speeds
    // 8 bytes (4 x int16)
    if (Serial.available() >= 8) {
      for (int i = 0; i < 4; i++) {
        fan[p].fan_speed[i] = read_serial_uint16();
      }
      Serial.write(s_ack);
      serial_func = 0;
    }
...
void ir_handler(uint16_t ir_number) {
  switch (ir_number)
  {
    ...
    case btn_bright_down:
         ...
         break;
    case btn_pause: fprint(18, true); break;
    case btn_power: fprint(19, true);
      ir_rgb_profile(0); break;
    ...
    case btn_red_up: fprint(40, true); break;
    case btn_green_up: fprint(41, true); break;
    case btn_blue_up: fprint(42, true); break;
    case btn_speed_up: fprint(43, true);
      change_led_rate(true); break;

    case btn_red_down: fprint(44, true); break;
    case btn_green_down: fprint(45, true); break;
    case btn_blue_down: fprint(46, true); break;
    ...
    case btn_diy_4: fprint(52, true); break;
    case btn_diy_5: fprint(53, true); break;
...
void change_led_rate(bool rate_up) {
  ...
  for (int i = 0; i < 3; i++) {
    if ((i + 1) == led_select or led_select == 0) {
      ...
      if (rate_up and steps > 1) {
        steps -= 1;  // Increase steps
      }
      else if (not rate_up and steps < 50) {
        steps += 1;  // Decrease steps
      }

      rgb[i].rate = 1 / steps;  // Calculate the new rate, likely to be some error but should work
...
// RGB channel 3 (power and HDD LED) - pwm 6, 7, 8
  ...
  for (int c = 0; c < 3; c++) {
    ...
    rgb[c].transition += rgb[c].rate;

    // SET PWM CHANNELS
//      set_pwm((c * 3) + 0, new_colour[0] * brightness);
//      set_pwm((c * 3) + 1, new_colour[1] * brightness);
//      set_pwm((c * 3) + 2, new_colour[2] * brightness);
    set_rgb(c, new_colour[0] * rgb[c].brightness, new_colour[1] * rgb[c].brightness, new_colour[2] * rgb[c].brightness);
  }
